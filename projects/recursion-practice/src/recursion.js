...
var factorial = function(n) {
  //base case
  // checking for the negative numbers and return null
  if(n < 0) {
    return null
  }
  ...
  //recursive case
  //multiply n by the value of the function call
...
var isEven = function(n) {
  //check if n is a negative number if it is change it a positive number
  if(n < 0){
    n = Math.abs(n);
  }
  //base case
...
var buildList = function(value, length, newArr = []) {
  ...
  //recursion case
  newArr.push(value);
  return buildList(value, length - 1).concat(newArr);
...
var capitalizeFirst = function(array, newArr = []) {
  ...
  newArr = capitalizeFirst(array.slice(1));
  ///used unshift method to add each capitalized the first letter of the word in the array
  newArr.unshift(array[0][0].toUpperCase() + array[0].slice(1));
  return newArr;
};
...
var compress = function(list, newArr = []) {
  ...
  newArr = compress(list.slice(1, list.length));
  if(list[0] !== newArr[0]){
...
var alternateSign = function(array, newArr = []) {
  ...
  return newArr.concat(alternateSign(array.slice(2)))
};
...
var numToText = function(str) {
  ...
  if (str[str.length - 1] === '1') {
    ...
    replaceStr = str[str.length - 1];
  }
  
  return  numToText(str.substring(0, str.length - 1)) + replaceStr;
};
